...
export class CSVImporter {
  ...
  async importFromCSV(csvContent: string, options: ImportOptions = {}): Promise<ImportResult> {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const rowNumber = i + 2; // +2 because Excel starts from 1 and we have header
  ...
  private async processRow(
    ...
    if (!row.sku) {
      throw new Error('Отсутствует SKU');
    }
    if (!row.title) {
      throw new Error('Отсутствует название товара');
    }
    if (!row.category) {
      throw new Error('Отсутствует категория');
    }
    if (!row.price) {
      throw new Error('Отсутствует цена');
    }
    if (!row.stock) {
      throw new Error('Отсутствует количество на складе');
    }
    ...
    if (isNaN(stock) || stock < 0) {
      throw new Error('Некорректное количество на складе');
    }
    ...
    if (!categoryId) {
      throw new Error(`Категория "${row.category}" не найдена`);
    }
    ...
    if (row.category) {
      ...
      if (categoryLower.includes('эконом') || categoryLower.includes('economy')) {
        productCategory = 'ECONOMY';
      } else if (categoryLower.includes('люкс') || categoryLower.includes('luxury') || categoryLower.includes('премиум')) {
    ...
    if (row.visibility) {
      ...
      if (visibilityLower === 'hidden' || visibilityLower === 'скрытый') {
        visibility = 'HIDDEN';
      } else if (visibilityLower === 'draft' || visibilityLower === 'черновик') {
        visibility = 'DRAFT';
      }
    }
