...
async function seed() {
  ...
  try {
    ...
    for (const productData of products) {
      ...
      const sku = generateSKU(createdCategories.find(c => c.id === productData.categoryId)?.name || 'PRODUCT', productData.title);
    ...
    const order = await db.order.create({
      data: {
        ...
        phone: customer.phone || '+7 (000) 000-00-00',
    ...
    const leads = [
      ...
      },
      {
        ...
        message: 'Хочу заказать индивидуальный пошив постельного белья',
        source: 'catalog',
        status: 'CONTACTED',
      },
    ];
